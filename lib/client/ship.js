...
ship = class extends ionizable(acceleratable(turnable(damageable(collidable(movable(spaceObject)))))) {
	...
	this.energy = this.properties.physics.energy;
	if (this.system) {
	...
	var outfitMass = this.outfits.map(function(outf) {
	    if (outf.meta.physics.freeMass) {
		return outf.meta.physics.freeMass * outf.count;
	    }
	    else {
		return 0;
	    }
	...
	this.properties.physics.freeMass = this.meta.physics.freeMass - outfitMass;
    }
	...
	if (this.properties.physics.maxSpeed < 0) {
	    this.properties.physics.maxSpeed = 0;
	}
	if (this.properties.physics.turnRate < 0) {
	    this.properties.physics.turnRate = 0;
	}
	...
	this.properties.physics.afterburnerFuel = null;
    }
	...
	if (this.properties.physics.afterburnerFuel == null) {
	    // No afterburner
 	    return false;
	}

	var cost = (this.properties.physics.afterburnerFuel / 1000) * delta;
	if (this.energy < cost) {
	    return false;
	}
	else {
	...
	if (this.properties.physics.energyRecharge) {
	    this.energy = Math.min(this.properties.physics.energy,
				   this.energy + this.properties.physics.energyRecharge * 60/1000 * delta);
	}
	...
	await this.sleep(this.properties.deathDelay * 1000);
