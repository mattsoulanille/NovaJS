...
Every package's `build` first runs nova's `scripts/clean_stale_dist.mjs` against its own `dist/`, which deletes `dist/` files whose source no longer exists. tsc's incremental builds never delete the outputs of files that were moved or renamed, so a checkout that pulls a source reorganization (e.g. `nova_plugin/*.ts` moving into domain directories) would otherwise keep the old flat `dist/**` copies beside the new ones — and jasmine's `dist/**/*_test.js` glob would load and run both. The cleaner runs right before `tsc` and after any step that generates a source file (novadatainterface's `build-defaults`, nova's `generate_build_version.mjs`): tsc's incremental build never re-emits an output it believes is up to date, so an output deleted while its generated source was missing would stay missing. Note that a turbo cache hit replays a build without running it, so a stale file that appears after switching back to an already-cached commit needs `npx turbo run build --force`. If you ever need to wipe by hand anyway (a broken tool, an interrupted build), it is `rm -rf packages/*/dist` or `npm run clean --workspaces --if-present`; the next `npm run build` recreates everything.
