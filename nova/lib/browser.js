...
socket.on('onconnected', async function(data) {
    ...
    global.allOutfits = await promiseMap(global.gd.meta.ids.Outfit, function(id) {
	return global.gd.data.Outfit.get(id);
    });

    global.allShips = await promiseMap(global.gd.meta.ids.Ship, function(id) {
	return global.gd.data.Ship.get(id);
    });
