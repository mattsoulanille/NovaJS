/**
 * Browser-safe resource-fork parsing: no fs/path here. index.ts adds the
 * node-only readResourceFork on top.
 */
...
/**
 * Parses resource-fork-format bytes (a Mac resource fork, or a file whose
 * data fork holds one), or a .rez container, into a ResourceMap. Pure:
 * usable in the browser (a File's bytes) as well as by readResourceFork.
 */
