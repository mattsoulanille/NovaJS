...
import { discoveryKeyFor } from '../nova_plugin/player/index.js';
import {
...
export function exportCheckpointFile(id: string, checkpointIndex: number,
    ...
    // checkpoint objects, and the export must not alias them. (A history
    // is plain JSON — it came out of JSON.parse — so a structured clone
    // is the same deep copy cloneJson makes, without pretending its
    // `unknown`-typed base and patch values are known to be JsonValue.)
    const truncated = structuredClone(truncateAfter(history, checkpointIndex));
