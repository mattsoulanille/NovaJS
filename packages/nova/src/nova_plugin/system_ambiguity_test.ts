...
describe('simulation system order ambiguities', () => {
    for (const platform of ['worker', 'node'] as const) {
        it(`are all declared away on the ${platform} platform`, async () => {
            ...
            // NPC traffic on: an NPC-only system pair that shares state
            // must be caught here too (review of #259, finding 1).
            const world: World = await makeSystem(systemId, gameData, platform);
