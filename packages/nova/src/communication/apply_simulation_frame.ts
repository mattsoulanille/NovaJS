...
import { stageEncodedComponentsGameData } from "../nova_plugin/core/index.js";
 ...
 * own `gameData` caches, so `applySimulationFrame` decodes them
 * synchronously. The worker staged them for its world; the display's
 * cache is its own. Call before every apply — a cache miss at apply
 * time drops the component with a warning.
 */
